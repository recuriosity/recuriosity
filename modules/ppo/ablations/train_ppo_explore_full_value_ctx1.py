"""No-camera-pose randmix PPO ablation: actor full memory, critic context=1."""
...
class NavAgentNoCameraPose(_BaseNavAgent):
    ...
    def forward(self, posed_images, last_only=False):
        ...
        actor_z = self._temporal_encode_full(frame_tok)
        critic_z = self._temporal_encode_ctx1(frame_tok)
    ...
    def forward_step(self, posed_image, time_idx: int, kv_caches):
        ...
        actor_z = self._temporal_step_full(frame_tok, kv_caches)
        critic_z = self._temporal_step_ctx1(frame_tok)
        logits = self.pi_head(actor_z)
...
def main(
    ...
    eval_every=10,
    checkpoint_path="checkpoints_saved/explore_full_value_ctx1.pt",
    ...
    def _wandb_init_with_nocam(*args, **kwargs):
        ...
        if isinstance(name, str):
            variant_tag = "actor_full_value_ctx1"
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--checkpoint_path",
        type=str,
        default="checkpoints_saved/explore_full_value_ctx1.pt",
    )
