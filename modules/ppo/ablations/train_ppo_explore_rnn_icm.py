"""Version-3 curiosity ablation: no-camera-pose PPO with LSTM backbone and frame-encoder ICM."""
...
from modules.ppo.ablations.train_ppo_explore_transformer_icm import (
    eval_rollout_video,
    ppo_update,
    rollout_with_cache,
)
...
class NavAgentNoCameraPoseRNNICMFrameEncoder(NavAgentLSTM):
    def __init__(
        self,
        *args,
...
NavAgent = NavAgentNoCameraPoseRNNICMFrameEncoder
...
def main(
    ...
    attn_window=64,
    eval_every=10,
    checkpoint_path="checkpoints_saved/explore_rnn_icm.pt",
    ...
    def _wandb_init_with_ablation(*args, **kwargs):
        ...
        if isinstance(name, str) and "nocam_rnn_icm_frameenc" not in name:
            kwargs["name"] = name.replace(
                "train_explore_base__",
                "train_explore_rnn_icm__",
                1,
            )
        return base_wandb_init(*args, **kwargs)

    _base.PoseProcess = PoseProcessNoCameraPose
    _base.NavAgent = NavAgentNoCameraPoseRNNICMFrameEncoder
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default="checkpoints_saved/explore_rnn_icm.pt",
    )
