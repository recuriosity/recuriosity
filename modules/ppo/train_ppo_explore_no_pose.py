"""Randmix PPO variant without camera-pose plucker channels in the policy input."""
...
def main(
    ...
    attn_window=64,
    eval_every=10,
    checkpoint_path="checkpoints_saved/explore_no_pose.pt",
    ...
    def _wandb_init_with_nocam(*args, **kwargs):
        ...
        if isinstance(name, str) and "nocam" not in name:
            kwargs["name"] = name.replace(
                "train_explore__",
                "train_explore_nocam__",
                1,
            )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--checkpoint_path",
        type=str,
        default="checkpoints_saved/explore_no_pose.pt",
    )
